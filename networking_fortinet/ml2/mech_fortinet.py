...
class FortinetMechanismDriver(driver_api.MechanismDriver):
    ...
    def delete_network_precommit(self, mech_context):
        ...
        if not namespace:
            return
        if network['id'] not in self.segments_to_delete:
            return
        for vlanid in self.segments_to_delete.pop(network['id']):
            inf_name = const.PREFIX['inf'] + str(vlanid)
            try:
    ...
    def handle_segment_change(self, rtype, event, trigger, context, segment):
        ...
        if (segment.get('segmentation_id') and
            ...
            if network_id not in self.segments_to_delete:
                self.segments_to_delete[network_id] = []
            self.segments_to_delete[network_id].append(vlanid)
