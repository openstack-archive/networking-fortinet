...
class FortinetMechanismDriver(driver_api.MechanismDriver):
    ...
    def delete_network_precommit(self, mech_context):
        ...
        if network["router:external"]:
            ...
            return
        if network["provider:segmentation_id"]:
            tenant_id = network['tenant_id']
    ...
    @registry.receives(cb_resources.SEGMENT, (events.PRECOMMIT_DELETE))
    def handle_segment_change(self, rtype, event, trigger, context, segment):
        LOG.debug("segment to delete is %(segment)s, context is %(ctx)s",
                  {'segment': segment, 'ctx': context})
        #if segment['segmentation_id'] and segment['network_type'] == "vlan":
        ...
        #    namespace = fortinet_db.query_record(plugin_context,
        #                            fortinet_db.Fortinet_ML2_Namespace,
        #                            tenant_id=tenant_id)
        #    if not namespace:
        #        return
        ...
        #    inf_name = const.PREFIX['inf'] + str(vlanid)
        #    try:
        #        utils.delete_vlanintf(self, plugin_context, name=inf_name,
        #                 vdom=namespace.vdom)
        #    except Exception as e:
        #        resources.Exinfo(e)
        #        raise ml2_exc.MechanismDriverError(
